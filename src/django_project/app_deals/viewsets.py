...
class DealsAPIView(APIView):
    ...
    def get_top_five_spending_clients(self):
        ...
        gems = (Deals.objects
                .filter(
            customer__in=top_five_customers.values_list('customer', flat=True))
                .values('customer', 'gem')
    ...
    def get(self, request):
        ...
        if not data:
            data = self.get_top_five_spending_clients()
            cache.set(self.CACHE_KEY, data, 60 * 5)
        #
        return Response({
            'response': data,
        })
